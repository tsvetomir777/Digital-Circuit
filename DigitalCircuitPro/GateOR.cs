...
namespace DigitalCircuitPro
{
...

    {
        ...
        //CONSTRUCTOR
        public GateOR(Point pos, int radius)
        :base(pos, radius)
        {
            this.input2 = -1;
        ...
        public override void DrawItself(Graphics gr, ImageList il)
        {
            ...
            gr.DrawImage(il.Images[2], this.Position.X - this.Radius, this.Position.Y - this.Radius, diameter, diameter);
        ...
        public override void removeConnection(Connection conn)
        {
            if (conn.ReceivingItem == this)
            {
                if (conn.ReceivingPoint == this.InputPos)
                    this.Input = -1;
                else if (conn.ReceivingPoint == this.Input2Pos)
                    this.Input2 = -1;
            }
